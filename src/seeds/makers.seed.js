...
const makersDocuments = makers.map(
  (maker) =>
    new Maker({
      ...maker,
      accepted: true,
...
const feedMakers = async () => {
    ...
    try {
        for (let i = 0; i < makersDocuments.length; i++) {
            const maker = makersDocuments[i];
            ...
            try {
                await mongoose
                  ...
                  .then(async () => {
                    ...
                    allMakers.length
                      ? await Maker.collection.drop()
                      : console.log(`the makers collection is already empty`);
                  })
                  ...
                  .catch((err) =>
                    console.log(`error at insertMany(makersDocuments): ${err}`)
                  );
              } catch (err) {
        ...
        console.log(`error at uploading makers images: ${err}`)
    }
};
